...
- On the default layer **`app.vue`**, the max-width gutter is omitted when
  ...
  `blank`, `auth`, or `preset-surface`. Use `fullBleed` for other layout names
  that need the same behavior.
...
} from '#layer/server/utils/accountDeletion'

const hooks: AccountDeletionHooks = {
  // Called after credential verification, before the local DB row is removed.
  // Throwing here aborts the entire deletion.
  beforeDelete: async (event, userId) => {
    await deleteSupabaseAuthUser(event, userId)
  },
}

await deleteCurrentUserAccount(event, user, body, hooks)
```
...
For guidance on keeping downstream apps aligned with this template over time,
see the **Downstream Drift and Staying Aligned** section in
