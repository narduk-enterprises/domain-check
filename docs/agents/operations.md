...
## GitHub PR Previews On `narduk`

This starter ships the repo-managed files the `narduk` preview host expects:
...
- `SITE_URL` defaults to that same loopback URL when the host does not inject a
  different origin.
- Health checks target `/api/health`, which already comes from the shared layer.

GitHub Packages auth for preview builds uses a BuildKit secret backed by
`GITHUB_TOKEN`. The token still needs repository read access plus
`read:packages`, but it is not passed as a Docker build arg and is not written
into image layers or committed files. If your packages token is stored under
`GITHUB_TOKEN_PACKAGES_READ` or `GH_TOKEN_PACKAGES_READ`, export it as
`GITHUB_TOKEN` before local preview smoke tests.

Existing apps can pick up preview asset updates later with:
