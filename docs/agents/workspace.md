...
5. Runs `pnpm install`.
6. Syncs the committed `.github/skills` mirror for GitHub coding agents. Refresh
   that mirror in the authoring repo with `pnpm run sync:github-skills` after
   changing your canonical local `~/.agents/skills` copy.

## Repo-Managed PR Preview Assets

This starter includes the repo-owned preview surface:

- root `.dockerignore`
- `deploy/preview/Dockerfile`
- `deploy/preview/docker-compose.yml`

Later `sync-template` runs keep those files up to date for existing apps.
`update-layer` alone does not, because the preview assets live in the app repo
rather than inside the shared layer.
